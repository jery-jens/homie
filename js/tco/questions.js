document.addEventListener("DOMContentLoaded", () => {
  ...
  const calcYearlyHypo = (perc, amount, months) => {
    ...
    return Number(costOneMonth * 12).toFixed(2);
  };
  ...
  administrationCostsMortgage.value = Number(500).toFixed(2);
  ...
  priceField.addEventListener("input", (e) => {
    ...
    notaryFeesField.value = Math.round(Number(notaryFee) + Number(administrativeCosts)).toFixed(2);
  ...
  loanAmountField.addEventListener("input", (e) => {
    mortgageRegistrationField.value = Number(((Number(e.target.value ?? 0) * 1.1) / 100) + ((Number(e.target.value ?? 0) * 1.1) / (100 * .3)) + 160.5 + (Number(e.target.value) <= 272727 ? 220 : 950)).toFixed(2);
  ...
  tcoForm.addEventListener("submit", (e) => {
    ...
    console.log(Math.round(Number(annualMortgageRepayment ?? 0) + Number(propertyTax ?? 0) + Number(insurances ?? 0) + Number(reparations ?? 0) + Number(management) + Number(commonCosts ?? 0)))
    console.log(annualMortgageRepayment, propertyTax, insurances, reparations, commonCosts);

    // window.location = "/" + window.location.pathname.split("/")[1] + "/tco-calculator/result" + "?credit=" + Math.round(Number(mortgageRegistration ?? 0) + Number(notaryMortgage ?? 0) + Number(administrationCostsMortgage ?? 0)) + "&on_purchase=" + Math.round(isNewProject.value ? Number(btw) + Number(landRegistry) + Number(notaryFees) : Number(registrationTax) + Number(landRegistry) + Number(notaryFees)) + "&after_purchase=" + Math.round(Number(energyRenovations ?? 0) + Number(otherRenovations ?? 0)) + "&yearly=" + Math.round(Number(annualMortgageRepayment ?? 0) + Number(propertyTax ?? 0) + Number(insurances ?? 0) + Number(reparations ?? 0) + Number(management) + Number(commonCosts ?? 0));
  });
});