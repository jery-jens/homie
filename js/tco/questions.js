document.addEventListener("DOMContentLoaded", () => {
  ...
  tcoForm.addEventListener("submit", (e) => {
    ...
    window.location = "/" + window.location.pathname.split("/")[1] + "/tco-calculator/result" + "?credit=" + Math.round(Number(mortgageRegistration ?? 0) + Number(notaryMortgage ?? 0) + Number(administrationCostsMortgage ?? 0)) + "&on_purchase=" + Math.round(isNewProject.value ? Number(btw) + Number(landRegistry) + Number(notaryFees) : Number(registrationTax) + Number(landRegistry) + Number(notaryFees)) + "&after_purchase=" + yearly;
  });
});